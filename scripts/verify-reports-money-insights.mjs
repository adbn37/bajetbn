...
const checks = [
  ...
  // v1.14.x reports moved from a single selectedMonth filter to
  // week/month/quarter/year period navigation anchored by selectedDate.
  [
    page,
    "type ReportPeriod = 'week' | 'month' | 'quarter' | 'year';",
    'Report period model',
  ],
  [page, 'selectedPeriod', 'Period filter'],
  [page, 'selectedDate', 'Period date anchor'],
  [page, 'activePeriodRange', 'Active report range'],
  [page, 'moveReportPeriod', 'Period navigation'],
  [page, 'aria-label="Report period"', 'Period selector UI'],
...
for (const phrase of forbidden) {
  ...
  assert.equal(
    pattern.test(page),
    false,
    `Beginner-facing report still contains: ${phrase}`,
  );
}
...
console.log(
  `Money reports checks passed (${checks.length} structural checks plus report calculations).`,
);
