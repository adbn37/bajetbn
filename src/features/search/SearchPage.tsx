...
export function SearchPage() {
  ...
  const allResults = useMemo<SearchResult[]>(() => [
    ...accounts.map((item) => ({
      ...
      route: '/accounts',
      searchable: [item.name, item.institution, item.institutionCode, item.type, item.classification].filter(Boolean).join(' '),
      accountId: item.id,
    ...
    ...transactions.map((item) => ({
      ...
      route: '/transactions',
      searchable: [item.counterparty, item.note, item.category, paymentMethodLabel(item.paymentMethod, item.paymentMethodLabel), item.type, item.transactionDate].filter(Boolean).join(' '),
    ...
    ...recurringTemplates.map((item) => ({
      ...
      searchable: [item.name, item.category, item.counterparty, item.note, paymentMethodLabel(item.paymentMethod, item.paymentMethodLabel), item.frequency, item.status].filter(Boolean).join(' '),
    ...
    ...commitments.map((item) => ({
      ...
      route: '/bills',
      searchable: [item.name, item.payee, item.categoryName, item.note, item.type].filter(Boolean).join(' '),
    ...
    ...goals.map((item) => ({
      ...
      route: '/goals',
      searchable: [item.name, item.note, item.status].filter(Boolean).join(' '),
    ...
    ...spaces.map((item) => ({
      ...
      searchable: [item.name, item.description, item.type].filter(Boolean).join(' '),
      spaceId: item.id,
