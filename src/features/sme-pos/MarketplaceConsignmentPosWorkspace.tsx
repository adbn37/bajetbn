...
export function MarketplaceConsignmentPosWorkspace({ space, settings, role, onChanged }: Props) {
  ...
  return <section className="sme-standard-pos-workspace marketplace-pos-workspace">
    ...
    {loading ? <div className="loading-panel">Loading records...</div> : <>
      {tab === 'sellers' && canManageSellers && <section className="panel sme-pos-module-panel">
        ...
        <div className="marketplace-seller-grid">{sellers.map((seller) => <article className="sme-pos-product-card" key={seller.id}>
          ...
          <small>{seller.soldQuantity} item(s) sold Â· Shop earned {formatMoney(seller.commissionEarnedMinor, seller.currency)}</small>
      ...
      {tab === 'listings' && <section className="panel sme-pos-module-panel">
        ...
        <div className="sme-pos-product-grid">{filteredListings.map((listing) => {
          ...
          return <article className={`sme-pos-product-card ${outOfStock ? 'out-of-stock' : ''}`} key={listing.id}>
            <div><span className="type-badge">{listing.sellerName}</span><h3>{listing.name}</h3><small>{conditionLabels[listing.condition]} Â· {listing.sku || listing.displayId}</small></div>
            {role !== 'stock_staff' && <strong>{formatMoney(listing.sellingPriceMinor, listing.currency)}</strong>}
            <p className={outOfStock ? 'stock-danger' : low ? 'stock-warning' : ''}>{outOfStock ? 'Out of stock' : `${listing.quantityOnHand} in stock${low ? ' Â· Low stock' : ''}`}</p>
      ...
      {tab === 'register' && <form className="sme-pos-checkout-layout" onSubmit={completeCheckout}>
        ...
        <section className="panel sme-pos-cart">
          ...
          <div className="form-stack compact">
            ...
            <label>Payment received in<select value={paymentAccountId} onChange={(event) => setPaymentAccountId(event.target.value)} required><option value="">Choose account</option>{paymentAccounts.map((item) => <option key={item.id} value={item.id}>{item.name} Â· {item.currency}</option>)}</select></label>
          ...
          <button className="button primary pos-complete-sale" type="submit" disabled={busy || !canCheckout || settings.status !== 'active' || !cartLines.length}>{busy ? 'Completing saleâ€¦' : settings.status !== 'active' ? 'POS is not active' : `Complete sale Â· ${formatMoney(totalMinor, settings.currency)}`}</button>
        </section>
      </form>}
      ...
      {tab === 'sales' && canViewSales && <div className="sme-pos-sales-section">
        ...
        <section className="panel"><div className="panel-heading"><div><h3>{role === 'cashier' ? 'My recent sales' : role === 'seller' ? 'My sales' : 'Recent Marketplace sales'}</h3><p>{role === 'seller' ? 'Only the part of each sale belonging to you is shown.' : 'Open a sale to view or print its receipt.'}</p></div></div><div className="sme-pos-sales-list">{sales.map((sale) => <button type="button" key={sale.id} onClick={() => setReceipt(sale)}><div><strong>{sale.receiptNumber}</strong><small>{sale.saleDate} Â· {sale.customerName || 'Walk-in customer'} Â· {sale.itemCount} item(s)</small></div><span className="status-badge posted">{sale.status}</span><strong>{formatMoney(role === 'seller' ? (sale.sellerEarningsMinor || 0) : sale.totalMinor - sale.returnedMinor, sale.currency)}</strong></button>)}</div>{!sales.length && <div className="empty-inline">No Marketplace sales available.</div>}</section>
      </div>}
    </>}
    ...
    {sellerForm && <Modal title={sellerForm === 'new' ? 'Add seller' : 'Edit seller'} onClose={() => !busy && setSellerForm(null)}><form className="form-stack" onSubmit={saveSeller}>
      ...
      <div className="modal-actions"><button className="button secondary" type="button" onClick={() => setSellerForm(null)}>Cancel</button><button className="button primary" type="submit" disabled={busy}>{busy ? 'Savingâ€¦' : 'Save seller'}</button></div>
    </form></Modal>}
