      ...
      {tab === 'listings' && <section className="panel sme-pos-module-panel">
        ...
        <div className="sme-pos-product-grid">{visibleInventoryListings.map((listing) => {
          ...
          >
            ...
            <div><span className="type-badge marketplace-seller-badge">{ownListing ? 'My stock' : listing.sellerName}</span><h3>{listing.name}</h3><small>{conditionLabels[listing.condition]} · {listing.sku || 'No SKU'}</small>{listing.barcode && <small>Barcode · {listing.barcode}</small>}</div>
      ...
      {tab === 'balance' && mySeller && <div className="sme-pos-sales-section">
        ...
        <section className="panel sme-pos-module-panel">
          ...
          <div className="sme-pos-product-grid">{visibleMySellerListings.filter((listing) => {
            ...
            >
              {listing.photoPath && <SmePosItemPhoto photoPath={listing.photoPath} name={listing.name} className="marketplace-inventory-thumb" />}
              <div><span className="type-badge marketplace-seller-badge">My stock</span><h3>{listing.name}</h3><small>{conditionLabels[listing.condition]} · {listing.sku || 'No SKU'}</small>{listing.barcode && <small>Barcode · {listing.barcode}</small>}</div>
