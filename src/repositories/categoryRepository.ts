...
export async function listAllCustomCategories(uid: string): Promise<TransactionCategory[]> {
...
export async function listCustomCategories(uid: string): Promise<TransactionCategory[]> {
  return (await listAllCustomCategories(uid)).filter((item) => !item.archivedAt);
}
