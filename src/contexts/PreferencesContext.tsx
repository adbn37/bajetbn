...
const guestAppearanceKey = 'bajetbn.guestAppearance.v1';
...
function readLocalPreferences(): Partial<PreferenceState> {
  try {
    ...
    const parsed = JSON.parse(value) as Partial<PreferenceState>;

    if (parsed.appearance !== undefined) {
      parsed.appearance = normalizeAppearance(
        parsed.appearance,
      );
    }
...
function readPendingGuestAppearance(): Appearance | null {
  try {
...
function clearPendingGuestAppearance() {
...
function profilePreferences(
  ...
  return {
    ...
    backgroundRemindersEnabled:
      profile?.backgroundRemindersEnabled
      ?? defaultPreferences.backgroundRemindersEnabled,

    dueSoonReminders:
      profile?.dueSoonReminders
      ?? defaultPreferences.dueSoonReminders,

    lateReminders:
      profile?.lateReminders
      ?? defaultPreferences.lateReminders,

    goalReminders:
      profile?.goalReminders
      ?? defaultPreferences.goalReminders,

    sharedPaymentNotifications:
      profile?.sharedPaymentNotifications
      ?? defaultPreferences.sharedPaymentNotifications,

    whatsappRemindersEnabled:
      profile?.whatsappRemindersEnabled
      ?? defaultPreferences.whatsappRemindersEnabled,

    browserPushEnabled:
      profile?.browserPushEnabled
      ?? defaultPreferences.browserPushEnabled,

    reminderDaysBefore:
      profile?.reminderDaysBefore
      ?? defaultPreferences.reminderDaysBefore,
  };
}

const PreferencesContext =
  createContext<PreferencesContextValue | undefined>(
    undefined,
  );
...
export function PreferencesProvider(
  ...
  const profileUid = useRef<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (pendingAppearance) {
      next.appearance = pendingAppearance;
    }
    ...
    if (
      ...
      if (
        ...
        clearPendingGuestAppearance();
        return;
      }

      void updateUserAppearance(
        user.uid,
        pendingAppearance,
      )
        .then(async () => {
        ...
        .catch((error) => {
          console.error(
            'Unable to sync the signed-out theme choice.',
            error,
          );
        });
    }
  ...
  const value =
    useMemo<PreferencesContextValue>(
      () => ({
        ...preferences,
        resolvedTheme,
        ...
        savePreferences:
          async (fullName) => {
            if (!user) {
              throw new Error(
                'Sign in before saving settings.',
              );
            }
