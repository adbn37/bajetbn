...
export function DashboardPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([listAccounts(user.uid), listSpaces(user.uid), listTransactions(user.uid), listBudgets(user.uid), listGoals(user.uid), listCommitments(user.uid)])
      .then(([nextAccounts, nextSpaces, nextTransactions, nextBudgets, nextGoals, nextCommitments]) => {
        setAccounts(nextAccounts); setSpaces(nextSpaces); setTransactions(nextTransactions); setBudgets(nextBudgets); setGoals(nextGoals); setCommitments(nextCommitments);
      })
  ...
  return <main className="page dashboard-page">
    <PageHeader eyebrow="Overview" title={`Good day, ${profile?.fullName?.split(' ')[0] || 'there'}`} description="See your accounts, spending plans, savings goals, bills, and instalments in one place." action={<Link className="button primary" to="/transactions">Add income or expense</Link>} />
    ...
    <section className="dashboard-grid"><article className="panel"><div className="panel-heading"><div><span className="eyebrow">Spaces</span><h2>Your money groups</h2></div><Link to="/spaces">View Spaces</Link></div><div className="space-chip-grid">{spaces.filter((item) => !item.archivedAt).slice(0, 6).map((space) => <Link to={`/spaces/${space.id}`} key={space.id} className="space-chip"><span className={`space-icon ${space.type}`}>{space.name.charAt(0)}</span><div><strong>{space.name}</strong><small>{space.type}</small></div></Link>)}</div></article>
